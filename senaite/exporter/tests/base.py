...
class SimpleTestLayer(PloneSandboxLayer):
    ...
    def setUpZope(self, app, configurationContext):
        ...
        # Load ZCML
        import senaite.exporter

        self.loadZCML(package=senaite.exporter)

        # Install product and call its initialize() function
        z2.installProduct(app, 'senaite.exporter')

    def setUpPloneSite(self, portal):
        super(SimpleTestLayer, self).setUpPloneSite(portal)

        # Apply Setup Profile (portal_quickinstaller)
        applyProfile(portal, 'senaite.exporter:default')
...
SIMPLE_TESTING = FunctionalTesting(
    ...
    name="senaite.exporter:SimpleTesting"
)
...
FUNCTIONAL_TESTING = FunctionalTesting(
    ...
    name="senaite.exporter:FunctionalTesting"
)
